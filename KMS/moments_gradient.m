...
if numgrad
    ...
    elseif DGP == 8
        ...
        Dg_eq = [Dg_eq ; -Dg_eq];
    elseif DGP == 9
        %% Example: Entry Game With Correlated Errors (in Kline & Tamer)
        % Parameter vector is
        % theta =
        ...
        % PRELIMINARY
        dim_p = KMSoptions.dim_p;
        suppX = KMSoptions.suppX;                   % Support for covariates [x1,x2], which
        ...
        % The rest needs to be modified based on DGP7
        for ii = 1:dim_suppX
            % Pick support point (include constant)
            ...
            %%%% Gradient WRT beta1,beta2,delta1,delta2 %%%%%%%%%%%%%%%%%%%%%%%
            % Gradients for moment inequalities for entry game (See Pg 34, eq 5.3)
            ...
            Dg4b2c = x2.*normpdf(x2*beta2+delta2).*normcdf((-x1*beta1-delta1-r*(x2*beta2+delta2))/sqrt(1-r^2))*pX ...
                +((-x2.*normpdf(-x2*beta2-delta2).*normcdf((-x1*beta1-rho*(-x2*beta2-delta2))/sqrt(1-rho^2)))...
                -(-x2.*normpdf(-x2*beta2).*normcdf((-x1*beta1-rho*(-x2*beta2))/sqrt(1-rho^2))))*pX;
            Dg4d1c = -normpdf(-x1*beta1-delta1).*normcdf((x2*beta2+delta2-r*(-x1*beta1-delta1))/sqrt(1-r^2))*pX;
            Dg4d2c = normpdf(x2*beta2+delta2).*normcdf((-x1*beta1-delta1-r*(x2*beta2+delta2))/sqrt(1-r^2))*pX ...
                -normpdf(-x2*beta2-delta2).*normcdf((-x1*beta1-rho*(-x2*beta2-delta2))/sqrt(1-rho^2))*pX;
            ...
            Dg2b1c = -x1.*normpdf(x1*beta1+delta1).*(normcdf((x2*beta2+delta2-rho*(x1*beta1+delta1))/sqrt(1-rho^2)))*pX;
            Dg2b2c = -x2.*normpdf(x2*beta2+delta2).*(normcdf((x1*beta1+delta1-rho*(x2*beta2+delta2))/sqrt(1-rho^2)))*pX;
            Dg2d1c = -normpdf(x1*beta1+delta1).*(normcdf((x2*beta2+delta2-rho*(x1*beta1+delta1))/sqrt(1-rho^2)))*pX;
            Dg2d2c = -normpdf(x2*beta2+delta2).*(normcdf((x1*beta1+delta1-rho*(x2*beta2+delta2))/sqrt(1-rho^2)))*pX;
    ...
    elseif DGP == 10
        %% Application: No correlated error
        % PRELIMINARY
        dim_p = KMSoptions.dim_p;
        suppX = KMSoptions.suppX;                   % Support for covariates [x1,x2], which
        psuppX = KMSoptions.psuppX;                 % Probability of support point occuring
        ...
        for ii = 1:dim_suppX
            ...
            Dg3b1 = x1.*normpdf(-x1*beta1-delta1) .*(1-normcdf(-x2*beta2))*pX;
            Dg3b2 = -x2.*normcdf(-x1*beta1-delta1).*normpdf(-x2*beta2)*pX;
            Dg3d1 = normpdf(-x1*beta1-delta1) .*(1-normcdf(-x2*beta2))*pX;
            Dg3d2 = 0;
            ...
            Dg4b2 = x2.*normcdf(-x1*beta1-delta1).*normpdf(-x2*beta2-delta2)*pX ...
                +x2.*normcdf(-x1*beta1).*(-normpdf(-x2*beta2-delta2)+normpdf(-x2*beta2))*pX;
            Dg4d1 = -normpdf(-x1*beta1-delta1).*(1-normcdf(-x2*beta2-delta2))*pX;
            Dg4d2 = normcdf(-x1*beta1-delta1).*normpdf(-x2*beta2-delta2)*pX ...
                +normcdf(-x1*beta1).*(-normpdf(-x2*beta2-delta2))*pX;
            ...
            Dg2b1 = -x1.*normpdf(-x1*beta1-delta1)     .*(1-normcdf(-x2*beta2-delta2))*pX;
            Dg2b2 = -x2.*(1-normcdf(-x1*beta1-delta1)) .*normpdf(-x2*beta2-delta2)*pX;
            Dg2d1 = -normpdf(-x1*beta1-delta1)     .*(1-normcdf(-x2*beta2-delta2))*pX;
            Dg2d2 = -(1-normcdf(-x1*beta1-delta1)) .*normpdf(-x2*beta2-delta2)*pX;
...
% Below are functions used for computing the gradient in DGP=7 and 9.
