...
KMSoptions.EAM_maxit    = 200;               % maximum # of iterations for EAM steps     (Program may terminate early if too small, e.g. less than 20).
KMSoptions.mbase        = 20;               % Initial points in EAM is mbase*dim_p + 1  (Smaller number = faster, but more sensitive.  Recommended values in [10,30])
KMSoptions.h_rate       = 1.5;              % Contraction rate of the parameter space.  (Larger number = faster, but more sensitive.  Recommend values in [1.3,3])
KMSoptions.h_rate2      = 1.45;              % Contraction rate for additional points.  Set to a number in (1,h_rate).
...
KMSoptions.EAM_thetadistort = 0.0001;        % Amount of distrtion of theta#.  We set it equal to EAM_obj_tol in our simulations.
...
KMSoptions.CVtol        = 1e-6;             % Tolerence on EAM critical value for determining "feasibility".
KMSoptions.toldirect1   = 1e-6;             % Tolerence for fmincon
KMSoptions.toldirect2   = 1e-8;             % Tolerence for fmincon
...
KMSoptions.EAM_maxviol_tol = 1/KMSoptions.B;
...
KMSoptions.FeasAll = 1;     % Equal to 0 to pass best, equal to 1 to pass all
...
% satisfied within tolerance level CVX_resid_tol.  Set this to [] in order 
% to set it to default value.  
KMSoptions.CVX_resid_tol = [];
...
KMSoptions.EI_points        = 20;                      % Number of points with positive expected improvement to draw
...
KMSoptions.EI_num           = 5;                       % Number of points to keep from expected improvement maximization.
KMSoptions.unif_num         = 5;                      % Number of points to uniformly draw to add to list on each iteration
...
KMSoptions.theta_true   = [];                                              % BCS profiling requires theta_true as a initial value of optimization
KMSoptions.W            = [];                                              % Also used in BCS profiling
KMSoptions.boundary     = [];                                              % Set this to 1 if ID set may touch the boundary of the parameter space.
KMSoptions.BCS_EAM      = [];                                              % Set it equal to 1 if you use EAM applied to BCS prifiling.
